...
def genomic_informative(row, groupd_genomic_keys_per_mm_dict, all_peps):
    ...
    def sites_nested_list_for_mm(groupd_genomic_keys_per_mm_dict,peptide):
        ...
        where each element in nested_list is a list of all genomic_keys of the specific mm_type that are representer by one instance of the peptide
        """
...
# =============================================================================    
    ...
    peps_df['genomic_keys'] = peps_df.apply(lambda row: genomic_keys_for_sites(row, sites_df), axis = 1)
    ...
    edited_peps_list = list(set(list(edited_peps['peptide'])))
