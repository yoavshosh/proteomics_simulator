...
#    all_peptides = 'E:/RNA_editing_Large_files/human_editom/proteomics_simulation/results_from_all_variants_c2t_a2g_3mc_6minl_5500maxm_20maxes/peps_from_all_variants_c2t_a2g_fasta.pickle'
    ...
    for i,m in enumerate(all_mm):
        det_sites = list(set([site for group in list(discovered_editing_sites_df['genomic_keys']) for site in group[i]]))
        detectable_sites_per_mm.update({mm:det_sites})
        print(str(len(det_sites)) + ' ' + mm + ' Editing sites discovered')
