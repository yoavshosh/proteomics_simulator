...
def read_non_editing_sites_wrt_coding_seqs(anovar_file):
    ...
    columns = ['variant_name', 'position_base0', 'position_base1', 'exon_number', 'chromosome', 'genomic_position_base0', 'genomic_position_base1', 'strand', 'genomic_key_base1','ucsc_id','HGNC_protein_id']
    data = []
    ...
    with open(anovar_file, "r") as f:
        ...
        for i, line in enumerate(content):
            line = line.replace('\n','')
            fields = line.split("\t")
            var_gene = fields[0].split(';') 
            fields.append(str(fields[4]+'_'+fields[7]+'_'+fields[6]))
            fields.append(var_gene[0])
            fields.append(var_gene[1])
            data.append(fields)
...
#    number_of_sites = 974
    ...
    while len(random_genomic_sites) < number_of_sites:
        ...
        if genomic_site not in random_genomic_sites:
            ...
            for index, row in variants_for_gene.iterrows():
                ...
                if edited_aa not in [original_aa,'*']:
                    recoding = True
            ...
            if recoding:
                ...
                if accept_site:
                    ...
                    print('Site ' + str(len(random_genomic_sites)) + ' appended: ' + genomic_site + ', variants: ' + str(len(variants_for_gene)))
    
    print('Sliceing dataframe for chosen sites')
    ...
    final_random_ag_sites_df.to_csv('/' + '/'.join(sites_file.split('/')[0:-1]) + '/' + output_name + '.txt', columns=columns, index_col = False, header = False)
