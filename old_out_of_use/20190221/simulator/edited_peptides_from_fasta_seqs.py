...
def edited_peptides_from_rna_seq(input_path,input_fasta, a2g_header_regex ,c2t_header_regex,
    ...
    for record in SeqIO.parse(input_path + input_fasta, "fasta"):
        ...
        if number_of_rna_peptides: #for each sequence (if peptides are generated from it) create a dataframe with multiindex (pep,seq,coor)
            ...
            pep_data_list = [(str(Seq(pep.seq, generic_rna).translate()).replace('I','X').replace('L','X'), str(record.id), '_'+str(pep.coo[0])+'_'+str(pep.coo[1]), (pep.a2g,pep.c2t), pep.N_terminus, pep.C_terminus, pep.sites_in_pep_range, pep.cancelled_cs_in_pep) for pep in rna_peptides_list]
            del(rna_peptides_list)
            #initial dataframe
            peps_df_temp = pd.DataFrame(pep_data_list,columns = ['peptide','seq_id','in_frame_coordinates_base0','editing_comb', 'N_terminus', 'C_terminus','sites_in_peptide_range','cancelled_cs_in_pep'])
            ...
            peps_df_temp = peps_df_temp.loc[:,('peptide','seq_id','in_frame_coordinates_base0','N_terminus','C_terminus','sites_in_peptide_range','cancelled_cs_in_pep')]
...
"""
excepts a final_peps_df in which peptides are stored and print to fasta file
"""
