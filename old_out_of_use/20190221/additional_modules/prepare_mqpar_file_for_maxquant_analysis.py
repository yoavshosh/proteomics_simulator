...
def write_mqpar_file(raw_files_path,raw_files,fasta_files,fdr,min_pep_len,max_mass,max_mc,mqpar_name):
    """
    ...
    with open(raw_files_path + 'mqpar_' + mqpar_name + '.xml', 'w') as f:
        ...
        f.write('         </fixedModifications>\n')
        f.write('         <enzymes>\n')
        f.write('            <string>Trypsin/P</string>\n')
        f.write('         </enzymes>\n')
...
if __name__ == '__main__':
    ...
    run_parser.add_argument('-o', dest='output_name', action='store', default = 'combined', help='mqpae and log files name and combined directory (MQ output) name')
    ...
    raw_files = [raw_files_path + f for f in files_in_raw_dir if (f[-4:]=='.raw' or f[-4:]=='.RAW')] #all raw files - full paths
    
    write_mqpar_file(raw_files_path,raw_files,fasta_files,fdr,min_pep_len,max_pep_mass,max_mc, mqpar_name = output_name)
