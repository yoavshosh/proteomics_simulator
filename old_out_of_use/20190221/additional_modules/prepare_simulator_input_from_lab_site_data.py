...
#        final_sequence, first_met, prot_start, prot_end = rethink_reading_frame(orf_sequence)
        ...
        if filter_sequences_with_stop_codons:
            full_protein_input_sequence = Seq(str(final_sequence), generic_rna).translate() #clear proteins which their contain stops whithin sequence
            full_protein_final_native_sequence = Seq(str(final_native_sequence), generic_rna).translate()  #clear proteins which thier final native sequence contain stops
            if str(full_protein_input_sequence).count('*') or str(full_protein_final_native_sequence).count('*'):
                bad_seq = True
