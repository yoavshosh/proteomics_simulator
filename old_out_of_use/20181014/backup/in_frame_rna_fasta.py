...
def create_in_frame_rna_file(out_path,input_path,fasta_input,a2g_editing_sites_df,strand_regex,start_nuc_regex,end_nuc_regex,c2t_editing_sites_df = [], only_recoding = False, filter_sequences_with_stop_codons = True):
    ...
    for record in SeqIO.parse(open(input_path + fasta_input, "r"), "fasta"):
        ...
        if filter_sequences_with_stop_codons:
            full_protein = Seq(str(orf_sequence), generic_dna).translate()
...
#    fasta_input = 'shahar_squ_orfs.fasta'
    ...
    if not os.path.exists(input_path + 'in_frame_fasta/'):
        os.makedirs(input_path + 'in_frame_fasta/')
    out_path = input_path + 'in_frame_fasta/'
...
#    atg_editing_sites_df = import_es_from_xls(input_path,xls_file,xls_sheet = xls_sheet, xls_columns_list = xls_columns_list, only_recoding = only_recoding)
    print('creating in-frame fasta from input fasta file (containing editing sites list relative to frame - base0)')
    create_in_frame_rna_file(out_path,input_path,fasta_input,atg_editing_sites_df,strand_regex,start_nuc_regex,end_nuc_regex,only_recoding = only_recoding, filter_sequences_with_stop_codons = filter_sequences_with_stop_codons)          
# =============================================================================
