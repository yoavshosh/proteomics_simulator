...
def read_samples_df(maxquant_files_list):
    ...
    columns = ['sample_name', 'sub_name', 'quantification', 'tissue', 'path']
    data = []
...
def search_edited_peptides(maxquant_file, proteomics_simulation_df, pos):   
    ...
    try:
        ...
        print(str(pos) + ' ' + str(len(mq_df)))
        print(pos,len(mq_df),discovered_peptides)
        return pos,len(mq_df),discovered_peptides
#        output.put((pos,len(mq_df),discovered_peptides))
    except:
        print(maxquant_file + '    Does not exist')
...
#    all_peptides_from_simulation = 'E:/RNA_editing_Large_files/human_editom/proteomics_simulation/results_from_sim_2mc_6minl_4600maxm_20maxes/peps_from_sim_fa.pickle'
    ...
    edited_peptides_df = edited_peptides_df[edited_peptides_df['genomic_informative']]
...
#    start parallel processing - maxquant results tables bs the simulation peptides table
    ...
    print('\nSorting results')
#    output_edited = sorted(output_edited, key=lambda x: None if x==None else x[0])
    ...
    if compare_all_peptides:
        ...
        print('\nSorting results')
        output_all_peps = sorted(output_all_peps, key=lambda x: None if x==None else x[0])
    
    

    maxquant_files_df['total_peptides'] = maxquant_files_df.apply(lambda row: 'File_not_found' if output_edited[row.name] == 'File_not_found' else output_edited[row.name][1], axis = 1)
    maxquant_files_df = maxquant_files_df[maxquant_files_df['total_peptides'] != 'File_not_found']
    print(maxquant_files_df)
    ...
    with open(peptide_file, "w") as pep_file:
        pep_file.write('sample_name\tsub_name\tquantification\ttissue\tseq_id\tgene_name\tpeptide\tsites_in_peptide_range\toriginal_version_detected\tgenomic_keys\tpath\n')
        for out in output_edited:
            if out != 'File_not_found':
                for i, row in out[2].iterrows():
                    ...
                    pep_file.write(maxquant_files_df.loc[out[0], 'sample_name'] + '\t' + maxquant_files_df.loc[out[0], 'sub_name'] + '\t' + maxquant_files_df.loc[out[0], 'quantification'] + '\t' + maxquant_files_df.loc[out[0], 'tissue'] + '\t' + row['seq_id'] + '\t' + row['HGNC_protein_id'] + '\t' + row['peptide'] + '\t' + str(row['sites_in_peptide_range']) + '\t' + str(original_version_found) + '\t' + str(row['genomic_keys'])  + '\t' + maxquant_files_df.loc[out[0], 'path'] + '\n')
