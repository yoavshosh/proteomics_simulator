...
def slice_sites_table_per_mm(anovar_df, mm_list = ['AG','CT']):
    ...
    for mm in all_mm:
        if mm in mm_list:
            ...
            anovar_slice.set_index('variant_name', inplace = True)
...
def read_editing_sites_wrt_coding_seqs(anovar_file):
    ...
    columns = ['variant_name', 'position_base0', 'position_base1', 'mm_type', 'chromosome', 'genomic_position_base0', 'genomic_position_base1', 'strand', 'aa_change', 'editing_level', 'genomic_key_base1','coding_key_base1']
...
if __name__ == '__main__':
    ...
    met_as_good_records = eval(arguments.met_as_good_records)
    stop_as_bad_records = eval(arguments.stop_as_bad_records)
    last_is_stop = eval(arguments.last_is_stop)
    
    if use_longest:
        assert refGene_file != 'no_file', 'use_longest is true but no refGene_file as input to determine longest variants per gene'
...
#    mm_types = ['AG']
    ...
    print('Reading and processing Sites tabel')
    print('mm types: ' + str(mm_types))
    sites = read_editing_sites_wrt_coding_seqs(sites_file)
    all_mm_df_dict = slice_sites_table_per_mm(sites, mm_list = mm_types)
    ...
    if use_longest:
        ...
        create_in_frame_rna_file_from_anovar_results_and_coding_mrna_seqs_final_sites_dfs(fasta_file,output_name,out_path,all_mm_df_dict,stop_as_bad_records,met_as_good_records,last_is_stop,variants_to_use=longest_variants)            
    else:
        print('Writing fasta file in the proteomics simulator format')
        create_in_frame_rna_file_from_anovar_results_and_coding_mrna_seqs_final_sites_dfs(fasta_file,output_name,out_path,all_mm_df_dict,stop_as_bad_records,met_as_good_records,last_is_stop)
